...
class KhanhArrayInt(BaseStruct):
    def __init__(self, lenArr, inArray=[], defaultData=0):
        ...
        for i in range(lenArr):
            if i < len(inArray):
                ...
                data.append(BaseElement(i, KhanhUint8(defaultData)))
        super().__init__(data)
...
class KhanhUint16(BaseStruct):
    def __init__(self, inUint=0):
        inArray = [inUint & 0xff, (inUint >> 8) & 0xff]
        super().__init__([BaseElement("value", KhanhArrayInt(2, inArray, 0))])
...
class KhanhUint32(BaseStruct):
    def __init__(self, inUint=0):
        ...
        super().__init__([BaseElement("value", KhanhArrayInt(4, inArray, 0))])
...
class KhanhUint64(BaseStruct):
    def __init__(self, inUint=0):
        ...
        super().__init__([BaseElement("value", KhanhArrayInt(8, inArray, 0))])
...
# Uint128
class KhanhUint128(BaseStruct):
    def __init__(self, inUint=0):
        inArray = [
            ((inUint >> i*8) & 0xff) for i in range(16)
        ]
        super().__init__([BaseElement("value", KhanhArrayInt(16, inArray, 0))])
...
# UintX
class KhanhUintX(BaseStruct):
    def __init__(self, lenArr, inUint=0):
        inArray = [
            (inUint >> i*8) & 0xff for i in range(lenArr)
        ]
        super().__init__([BaseElement("value", KhanhArrayInt(lenArr, inArray, 0))])
...
# String 64bit data
class KhanhString64(KhanhArrayInt):
...
class KhanhArrayStruct(BaseStruct):
...
class KhanhVectorStruct(BaseStruct):
    def __init__(self, maxlen, lamdaCreateObj, inArray=[], UintLen=KhanhUint32(0)):
        if not callable(lamdaCreateObj):
            raise Exception("Error lamdaCreateObj is not function")
        if not isinstance(lamdaCreateObj(), BaseStruct):
            raise Exception("Error struct is not BaseStruct")
        ...
        data.append(BaseElement("data", KhanhArrayStruct(
    ...
    def __init__(self, lenArr, inArray=[]):
        ...
        for i in range(lenArr):
            if i < len(inArray):
                ...
                data.append(BaseElement(i, KhanhUint8(0)))
        super().__init__(data)
...
# KhanhStringUTF16
class KhanhStringUTF16(KhanhArrayStruct):
    def __init__(self, lenArr: int, inString: str = ""):
        ...
        inArray = []
        for i in range(len(inString)):
            inArray.append(KhanhUint16(ord(inString[i]) + 1))
        super().__init__(lenArr, lambda: KhanhUint16(0), inArray)
...
# KhanhDate
class KhanhDate(BaseStruct):
    def __init__(self, day=0, month=0, year=0):
        super().__init__([
            BaseElement("day", KhanhUint8(day)),
            BaseElement("month", KhanhUint8(month)),
            BaseElement("year", KhanhUint16(year))
        ])
...
# KhanhPublicKey
class KhanhPublicKey(BaseStruct): # Fix length 32 bytes
    def __init__(self, inStringB58: Optional[Union[str, bytes]] = None):
        ...
        if inStringB58 is not None:
            ...
            for i in range(32):
                data.append(BaseElement(i, KhanhUint8(inStringB58[i])))
        else:
            for i in range(32):
                data.append(BaseElement(i, KhanhUint8(0)))
